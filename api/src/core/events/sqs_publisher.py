...
class SQSEventPublisher:
    ...
    async def _ensure_queues_exist(self):
        """Get URLs for manually created queues only"""
        loop = asyncio.get_event_loop()
        
        # Use the queue URL from environment if provided
        queue_url = os.getenv('SQS_QUEUE_URL')
        if queue_url:
            # Map all event types to this one queue
            for event_type in self.queue_config.keys():
                self.queue_urls[event_type] = queue_url
            
            logger.info(f"✅ Using queue URL from environment: {queue_url}")
            return
        
        # Fallback: try to get queue by name
        main_queue_name = "video-processing-queue"
        
        try:
            response = await loop.run_in_executor(
                None,
                lambda: self.sqs_client.get_queue_url(QueueName=main_queue_name)
            )
            ...
            logger.info(f"✅ Using queue: {main_queue_name}")
            
        except Exception as e:
            logger.error(f"❌ Failed to find queue {main_queue_name}: {e}")
            raise Exception(f"Queue {main_queue_name} not found. Please create it first.")
    ...
    async def _ensure_queues_exist(self):
        ...
        # Only check for the main processing queue
        main_queue_name = "video-processing-queue"
        ...
        try:
            ...
            # Map all event types to this one queue for now
            for event_type in self.queue_config.keys():
                self.queue_urls[event_type] = queue_url
